...
int main() {
  ...
  _io_ports[M6812_PORTG] |= M6812B_PG7; // Lo que sale hacia fuera, es un pin de salida
  ...
  _io_ports[M6812_SC0CR2] = (M6812B_RIE | M6812B_TE | M6812B_RE); // Pagina 7 seriales
...
// Cada vez que reciba un caracter que el led rojo cambie, si está on -> off, off -> on
...
void __attribute__((interrupt))vi_sci0(void) {
  ...
  if(_io_ports[M6812_SC0SR1] & M6812B_RDRF) {
    ...
    /*Encendemos led o apagamos*/ 
    _io_ports[M6812_PORTG] |= M6812B_PG7; // Lo que sale hacia fuera, es un pin de salida
    
    //if (_io_ports[M6812_PORTG] & M6812B_PG7) {
